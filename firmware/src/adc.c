...
/*
 *  Reads ADC results and stores them to an array for later use.
 */
void ADC_ResultHandler(uintptr_t context)
{
    ...
    if(adc_halfSec == adc_PERIOD*2){ //Can't be less than 3 adc_halfSec, 1 second is too fast
        if(ADCi<adc_ASIZE){
            ...
            five_v[ADCi]=((float)adc_arr[2] * ADC_VREF / ADC_MAX_COUNT)*(4.964/2.988);//uses measured resistor values from voltage divider on board
            twelve_v[ADCi]=((float)adc_arr[0] * ADC_VREF / ADC_MAX_COUNT)*(4.096/1.093);//^
            ftune[ADCi]=(float)adc_arr[3] * (ADC_VREF / ADC_MAX_COUNT);
            
            ADCi++;
            if(ADCi==(adc_ASIZE-1) && !adc_full){//checks if arrays are full
                adc_full=1;
            }
        }
        if(ADCi==adc_ASIZE){//starts over ADC index (ADCi) when reaching end of the array 
            ADCi=0;
        }
        adc_halfSec = 0;
    }
...
/*
 *  Prints ADC voltages for the instant its called.
 */
void get_ADC(void){
